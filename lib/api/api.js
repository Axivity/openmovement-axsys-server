/**
 * Created by Praveen on 09/09/2015.
 *
 * @flow
 */

import * as events from 'events';

import moment from 'moment';

import { Payload, AxsysError } from './payload';
import * as serial from '../comms/serial';
import * as stringUtils from '../utils/string-utils';
import { AX_ON_DATA } from '../constants/event-name-constants';
import { AttributesAPI } from './attributes-api';
import { ProcessAPI } from './process-api';
import { FilesAPI } from './files-api';
import * as eventNameConstants from '../constants/event-name-constants';
import * as actionCreators from '../action-creators/cache-action-creator';

export class APIInterface {

    // TODO: Decide on how to refactor it to have same interface as client
    //       Possibly create an index.js file that delegates to APIInterface?
    //       So instantiate this class and proxy all the requests to this class from index.js - hmmm

    //constructor(onDeviceAdded,
    //            onDeviceRemoved,
    //            onConnected,
    //            onDisconnected,
    //            onDataReceived,
    //            clientKey) {
    //}

    constructor(store, eventBus, dataHandler, attributesPublishHandler) {
        this.ports = {};
        this.store = store;
        this.eventBus = eventBus;
        this.attributesCacheApi = new AttributesAPI(store, this._onDataPublished.bind(this));
        this.dataHandler = dataHandler;
        this.attributesPublishHandler = attributesPublishHandler;
        this.processApi = new ProcessAPI(store);
        this.filesApi = new FilesAPI(store);
        this.consumeDeviceAddedOrRemovedEventsIntoCacheStore();
    }

    _onDataPublished(changedState) {
        let payload = new Payload(null, changedState, null);
        this.attributesPublishHandler(payload);
    }

    getDevices() {
        let devices = this.attributesCacheApi.getAll();
        let payload = new Payload(null, devices, null);
        return payload;
    };

    getFiles() {
        let files = this.filesApi.getAll();
        let payload = new Payload(null, files, null);
        return payload;
    };

    connect(options, callback) {
        let path = stringUtils.parseSerialPath(options.path);
        let emitter = new events.EventEmitter();
        options.emitter = emitter;

        let serialPort = new serial.SerialComms(options);
        emitter.on(AX_ON_DATA, this.onWritten.bind(this));

        serialPort.openPath((err) => {
            if(err) {
                let axErr = new AxsysError(err.name, err.message, '');
                let payload = new Payload(axErr, null, options.path);
                callback(payload);

            } else {
                let payload = new Payload(null, {}, options.path);
                callback(payload);
            }
        });

        this.ports[path] = serialPort;
    };

    disconnect(options, callback) {
        let path = stringUtils.parseSerialPath(options.path);
        let serialPort = this.ports[path];
        serialPort.closePath(path, (err) => {
            if(err) {
                let axErr = new AxsysError(err.name, err.message, '');
                let payload = new Payload(axErr, null, options.path);
                callback(payload);

            } else {
                let payload = new Payload(null, {}, options.path);
                callback(payload);
            }
        });
    };

    onWritten(response) {
        console.log('Same event raised');
        console.log(response);

        let data = response.data;
        //let path = stringUtils.parseSerialPath(response.options.path);
        let payload = new Payload(null, {
            'buffer': data,
            'dateTime': moment().format(),
            'options': response.options
        }, response.options.path);
        this.dataHandler(payload);
    };

    write(options, callback) {
        let path = stringUtils.parseSerialPath(options.path);
        let serialPort = this.ports[path];
        serialPort.write(options, (err, response) => {

            if(err) {
                let axErr = new AxsysError(err.name, err.message, '');
                let payload = new Payload(axErr, null, options.path);
                callback(payload);

            } else {
                let payload = new Payload(null, response, options.path);
                callback(payload);
            }
        });
    }

    publish(devicePath, attributeKey, attributeVal) {
        this.attributesCacheApi.publish(devicePath, attributeKey, attributeVal);
        let payload = new Payload(null, {}, null);
        return payload;
    }

    createProcess(options) {
        let processCreated = this.processApi.create(options);
        if(processCreated) {
            return new Payload(null, {}, options.path);

        } else {
            return new Payload(new Error('Cannot start process'), null, options.path);

        }

    }

    consumeDeviceAddedOrRemovedEventsIntoCacheStore() {
        var self = this;
        this.eventBus.subscribe(eventNameConstants.AX_DEVICE_ADDED, function(device) {
            let path = stringUtils.removeWindowsPrefixToSerialPath(device.port);
            let devicePath = stringUtils.constructSerialPath(path);
            self.store.dispatch(actionCreators.createDeviceWithAttributes({
                devicePath: devicePath,
                deviceAttributes: device
            }));
            console.log('Added new device');
        });

        this.eventBus.subscribe(eventNameConstants.AX_DEVICE_REMOVED, function(device) {
            let path = stringUtils.removeWindowsPrefixToSerialPath(device.port);
            let devicePath = stringUtils.constructSerialPath(path);
            self.store.dispatch(actionCreators.removeDeviceWithAttributes({
                devicePath: devicePath
            }));
            console.log('Removed a device');
    });
}

}