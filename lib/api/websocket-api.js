...
import { hasError, Payload } from './payload';
import * as stringUtils from '../utils/string-utils';

export class WebsocketAPI {
    constructor(store, eventBus, dataPublisher) {
    ...
    getFiles(ws) {
        ...
        let response = JSON.stringify({
            'event': constants.AX_CLIENT_FILES_GET_ALL,
            'data': files
        });
        console.log(response);
        ws.send(response);
     ...
     * @param options
     */
    createProcess(ws, options) {
        let payload = this.api.createProcess(options);
        ws.send(JSON.stringify({
            'event': constants.AX_CREATE_PROCESS,
            'data': payload
        }));
    }
...
export function websocketSetup(wss, store, eventBus) {
    ...
    let webSocketAPI = new WebsocketAPI(store, eventBus, publishDataToConnectedSockets);
    ...
    wss.on('connection', (ws) => {
        ...
        ws.on('message', (message) => {
            ...
            opts[constants.AX_CREATE_PROCESS] = function() {
                webSocketAPI.createProcess(ws, msg.data);
            };

            opts[constants.AX_CLIENT_FILES_GET_ALL] = function() {
                webSocketAPI.getFiles(ws);
            ...
            opts[constants.AX_CLIENT_REGISTER] = function() {
                ...
                let payload = new Payload(null, {status: 'SUCCESS'}, null);
